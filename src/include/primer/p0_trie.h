...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept
    ...
    for (auto &c : other_trie_node.children_) {
      children_.insert(std::make_pair(c.first, std::move(c.second)));
    }
  }
...
class Trie {
 ...
 private:
  bool Remove(const std::string &key, std::unique_ptr<TrieNode> *parent, size_t idx) {
    if (key.empty() || idx >= key.size()) {
      return false;
    }

    auto child = (*parent)->GetChildNode(key[idx]);
    if (child == nullptr) {
      return false;
    }
    ...
    if (key.size() - 1 == idx) {
      if ((*child)->IsEndNode()) {
        if ((*child)->HasChildren()) {
          std::unique_ptr<TrieNode> trie_node = std::make_unique<TrieNode>(std::move(**child));
          trie_node->SetEndNode(false);
          (*parent)->RemoveChildNode(key[idx]);
          (*parent)->InsertChildNode(key[idx], std::move(trie_node));
        } else {
          (*parent)->RemoveChildNode(key[idx]);
        }
        return true;
    ...
    auto res = Remove(key, child, idx + 1);
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (size_t i = 0; i < key.size() - 1; i++) {
      next_node = (*node)->GetChildNode(key[i]);
      if (next_node == nullptr) {
        next_node = (*node)->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
        assert(next_node != nullptr);
      }
      node = next_node;
    }

    next_node = (*node)->GetChildNode(key[key.size() - 1]);

    if (next_node == nullptr) {
      (*node)->InsertChildNode(key.back(), std::make_unique<TrieNodeWithValue<T>>(key.back(), value));
      latch_.WUnlock();
      return true;
    }
    if (next_node != nullptr && (*next_node)->IsEndNode()) {
      latch_.WUnlock();
      return false;
    }

    std::unique_ptr<TrieNodeWithValue<T>> value_node =
        std::make_unique<TrieNodeWithValue<T>>(std::move(*(*next_node)), value);
    (*node)->RemoveChildNode(key.back());
    (*node)->InsertChildNode(key.back(), std::move(value_node));
    latch_.WUnlock();
    return true;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    *success = true;
    auto value = value_node->GetValue();
    latch_.RUnlock();
    return value;
  }
};
